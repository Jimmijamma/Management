...
if __name__ == '__main__':
    ...
    for INTER_ARRIVAL in range(1000,100,-10):
        ...
        probability_losses.append(100.0*server.n_discarded/car_arrival.n_arrivals)
        
        
        
        print "mean response time: " + str(mean_rp) + "		rho: " + str(ro)
        print "n. of packets served: " + str(len(server.response_time))
        print "n. arrivals: " + str(car_arrival.n_arrivals)
        print "n. discarded: "+  str(server.n_discarded)
        print "probability loss: " + str(100.0*server.n_discarded/car_arrival.n_arrivals)
    ...
    plt.xlabel("rho")
    plt.legend(loc = 2)
    plt.savefig('Response_time_vs_rho')
    plt.close()
    ...
    plt.xlabel("rho")
    plt.legend(loc = 2)
    plt.savefig('Buffer_occupancy_vs_rho')
    plt.close()
    ...
    #plot della probability loss
    plt.plot(ros, probability_losses, label = "Probability loss")
    plt.ylabel("Loss Probability")
    plt.xlabel("rho")
    plt.legend(loc = 2)
    plt.savefig('Probability_loss_vs_rho')
    plt.close()
